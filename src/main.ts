    ...
    return createPlugin(
        ...
        function rule(_: any, options: object | null = null) {
            return (root: postcss.Root, result: postcss.Result) => {
                ...
                const pathToFile: string = Util.removeLastSegmentsFromPath(__dirname, 3);
                const {css, propertiesWhitelist}: PluginOption = options as PluginOption;
                ...
                function seedMaps(statement: postcss.Rule, propertiesMap: Map<string, Array<any>>, selectorsMap: Map<string, any>) {
                    ...
                    statement?.nodes?.forEach((node: postcss.ChildNode) => {
                        ...
                        if (propertyWhitelistSet && (!propertyWhitelistSet.has(declaration.prop))) {
                            return;
                        }
                ...
                function seedAtomicsMap(atomicsMap: Map<string, Map<string, any>>, rule: postcss.Rule) {
                ...
                function reportWhenAtomicIsPresent(atomicsMap: Map<string, Map<string, any>>) {
                ...
                function handlePropertyWhiteList(propertiesWhitelist: Array<string>): Set<string> {
